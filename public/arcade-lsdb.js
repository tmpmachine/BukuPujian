...
const arclsdb = function(storageName, rootObject) {
...
(function() {
  ...
  arclsdb.prototype.deepCheck = function(data, root, firstDive) {
    if (data === null)
  ...
  arclsdb.prototype.getStorageData = function(name, defaultValue) {
  	return (!window.localStorage.getItem(name)) ? defaultValue : window.localStorage.getItem(name);
  };
  
  arclsdb.prototype.save = function() {
    window.localStorage.setItem(this.storageName, JSON.stringify(this.data));
  };
  
  arclsdb.prototype.reset = function() {
    window.localStorage.removeItem(this.storageName);
    this.data = JSON.parse(JSON.stringify(this.root.root));
  };
  
  arclsdb.prototype.new = function(keyName, values) {
